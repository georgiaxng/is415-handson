...
resale_sf <- resale_sf %>%
  mutate(housing_type = case_when(
    ...
    floor_area_sqft > 1399 & floor_area_sqft <= 1615 ~ "EA/EM/6 Rm",  # Closer to 1615 sqft
  ))
...
###  2.3.3 Time Period Selection for Analysis
...
tm_shape(mpsz)+
tm_polygons()+
tm_shape(resale_2023_sf)+
tm_dots()

{r}
tm_shape(mpsz)+
tm_polygons()+
tm_shape(resale_2022_sf)+
tm_dots()
