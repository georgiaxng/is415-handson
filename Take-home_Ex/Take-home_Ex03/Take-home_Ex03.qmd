...
Similarly here, we will use `st_read` to read the geojson information, however since the columns values are in the format of html code of '<th><td>' etc we will need to use a function to apply a regex expression in order to extract the name of the hawker centres.
...
For my part I am in charge of the explanatory data analysis.

## 5.1 Histogram

![](images/clipboard-2250997801.png){width="676"}

## 5.2 Chloropleth Map

![](images/clipboard-2681238715.png){width="683"}

## 5.3 Scatterplot

# 6. Histograms

For the histogram, the user will be given the options to:

-   Filter by (Only for plot for continuous variables): `month_year`, `flat_type`, `town`

-   Plot different variables: Categorical & Continuous
...
# 7. Chloropleth Mapping

For the chloropleth map, the user will be given the options to:

-   Filter by: `month_year` and `flat_type`

-   Plot different variables: Median Monthly Rent, Rented Flat Count

::: callout-note
Note: As Kallang/Whampoa does not match any towns in `mpsz_sf`, we will map it to Kallang. On the other hand for the town 'Central', since we are not able to make any deduction based on this naming and it also does not match any values in `mpsz_sf`, we can only remove it from the chloropleth mapping.
:::
...
tm_shape(mpsz_sf)+
  tm_polygons()+
  tm_shape(rental_sf %>% filter(flat_type=='3-ROOM'))+
  tm_dots()
```
