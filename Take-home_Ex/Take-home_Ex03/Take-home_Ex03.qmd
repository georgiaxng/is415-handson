...
```{r}
pacman::p_load(sf, sfdep, tmap, tidyverse, RColorBrewer, ggplot2)
```
...
columns_to_remove <- c("floor_area_sqm", "flat_model", "lease_commence_date", "remaining_lease", "blk_no", "road_name", "building", "address", "lease_commence_date_r", "postal")
...
### 2.3.4 Time Period Selection for Analysis
...
```{r}
test <- resale_5yr_sf %>% filter(is.na(remaining_lease_range))
test
```
...
```{r}
unique(resale_5yr_sf$storey_range)
```
