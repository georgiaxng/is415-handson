...
To confirm the observed spatial patterns above, a hypothesis test (i.e. Monte Carlo simulation test) will be conducted. For this, the hypothesis and test are as follows:
...
### 5.2.4 Violence Against Civilians
...
This chunk of code creates a Sagaing owin object, converts acled_sf into a ppp object and plots the a map displaying only conflict events in Sagaing Region.
...
for (i in seq_len(nrow(battle_data_sf))) {
  ...
  # Save the result in the list with a unique name
  name <- paste0(current_year, "_Q", current_quarter)
  sagaing_battle_G_CK_list[[name]] <- G_CK.csr
}

write_rds(sagaing_battle_G_CK_list, "data/rds/sagaing_battle_G_CK_list.rds")

```

```{r}
#| echo: false # so that code chunk is not seen
sagaing_battle_G_CK_list <- read_rds( "data/rds/sagaing_battle_G_CK_list.rds")
```
...
par(mfrow=c(4,4))
# Plotting
for(i in seq_along(sagaing_battle_G_CK_list)) {
  g_ck <- sagaing_battle_G_CK_list[[i]]
  plot(g_ck, xlim=c(0,10000), main=names(sagaing_battle_G_CK_list)[i])
}
```

#### 5.3.2.3 Explosions/ Remote Violence
...
# Initialize an empty list to store the results
sagaing_explosions_G_CK_list <- list()
...
for (i in seq_len(nrow(explosions_data_sf))) {
  ...
  # Save the result in the list with a unique name
  name <- paste0(current_year, "_Q", current_quarter)
  sagaing_explosions_G_CK_list[[name]] <- G_CK.csr
}
write_rds(sagaing_explosions_G_CK_list, "data/rds/sagaing_explosions_G_CK_list.rds")

```

```{r}
#| echo: false # so that code chunk is not seen
sagaing_explosions_G_CK_list <- read_rds( "data/rds/sagaing_explosions_G_CK_list.rds")
```
...
par(mfrow=c(4,4))
# Plotting
for(i in seq_along(sagaing_explosions_G_CK_list)) {
  g_ck <- sagaing_explosions_G_CK_list[[i]]
  plot(g_ck, xlim=c(0,10000), main=names(sagaing_explosions_G_CK_list)[i])
}
```

#### 5.3.2.4 Strategic Developments
...
# Initialize an empty list to store the results
sagaing_strategic_G_CK_list <- list()
...
for (i in seq_len(nrow(strategic_data_sf))) {
  ...
  # Save the result in the list with a unique name
  name <- paste0(current_year, "_Q", current_quarter)
  sagaing_strategic_G_CK_list[[name]] <- G_CK.csr
}
write_rds(sagaing_strategic_G_CK_list, "data/rds/sagaing_strategic_G_CK_list.rds")
```

```{r}
#| echo: false # so that code chunk is not seen
sagaing_strategic_G_CK_list <- read_rds( "data/rds/sagaing_strategic_G_CK_list.rds")
```
...
par(mfrow=c(4,4))
# Plotting
for(i in seq_along(sagaing_strategic_G_CK_list)) {
  g_ck <- sagaing_strategic_G_CK_list[[i]]
  plot(g_ck, xlim=c(0,10000), main=names(sagaing_strategic_G_CK_list)[i])
}
```

#### 5.3.2.5 Violence against Civilians
...
# Initialize an empty list to store the results
sagaing_violence_G_CK_list <- list()
...
for (i in seq_len(nrow(violence_data_sf))) {
  ...
  # Save the result in the list with a unique name
  name <- paste0(current_year, "_Q", current_quarter)
  sagaing_violence_G_CK_list[[name]] <- G_CK.csr
}
write_rds(sagaing_violence_G_CK_list, "data/rds/sagaing_violence_G_CK_list.rds")
```

```{r}
#| echo: false # so that code chunk is not seen
sagaing_violence_G_CK_list <- read_rds( "data/rds/sagaing_violence_G_CK_list.rds")
```
...
par(mfrow=c(4,4))
# Plotting
for(i in seq_along(sagaing_violence_G_CK_list)) {
  g_ck <- sagaing_violence_G_CK_list[[i]]
  plot(g_ck, xlim=c(0,10000), main=names(sagaing_violence_G_CK_list)[i])
}
```
...
```{r}
  ...
  # Create a spatio-temporal point pattern (ppp is the spatial part, time is added separately)
  coords_matrix <- st_coordinates(grped_acled_quartered_sf)
   x_coords <- coords_matrix[, "X"] /1000
  y_coords <- coords_matrix[, "Y"] /1000
...
for (i in seq_len(nrow(battle_data_sf))) {
  ...
  filtered_ppp <- filtered_ppp[sagaing_owin]
  L_ck.csr <- envelope(filtered_ppp, Lest, nsim = 99, rank = 1, glocal=TRUE)
  
  # Save the result in the list with a unique name
  name <- paste0(current_year, "_Q", current_quarter)
  sagaing_battle_L_CK_list[[name]] <- L_ck.csr
}

write_rds(sagaing_battle_L_CK_list, "data/rds/sagaing_battle_L_CK_list.rds")

```

```{r}
#| echo: false # so that code chunk is not seen
sagaing_battle_L_CK_list <- read_rds( "data/rds/sagaing_battle_L_CK_list.rds")
```
...
par(mfrow=c(4,4))
# Plotting
for(i in seq_along(sagaing_battle_L_CK_list)) {
  g_ck <- sagaing_battle_L_CK_list[[i]]
  plot(g_ck, . - r ~ r, xlab="d", ylab="L(d)-r", main=names(sagaing_battle_L_CK_list)[i])
}
...
```{r}
  k_ck <- sagaing_battle_K_CK_list[[3]]
  plot(k_ck, . - r ~ r, xlab="d", ylab="k(d)-r", main=names(sagaing_battle_K_CK_list)[3])
```
